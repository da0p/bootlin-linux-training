...
static irqreturn_t serial_irq_handler(int irq, void *dev_id)
{
	...
	dev_dbg(serial->miscdev.parent, "received %c\n", ch);
		...
		size_t sz, loff_t *ppos)
{
	...
	for (i = 0; i < sz; i++) {
		...
		dev_dbg(serial->miscdev.parent, "write %c\n", c);
...
static int serial_probe(struct platform_device *pdev)
{
	 ...
	 * don't:
	 * If CONFIG_DEBUG_FS is not set, the other debugfs functions expand to
	 * empty code, so the return value from debugfs_create_dir will be
	 * ignored.
	 * Otherwise, the subsequent debugfs functions check IS_ERROR(parent)
	 * and return without doing anything in case the creation of the parent
	 * failed.
	 */
	serial->debugfs_dir = debugfs_create_dir(serial->miscdev.name, NULL);
	debugfs_create_u32("counter", 0644, serial->debugfs_dir, &serial->counter);
